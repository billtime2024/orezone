...
export default {
    ...
    data() {
        return {
            form: useForm({
                provider_type: this.provider.provider_type || 'homemade',
                ...
                pincode: this.provider.pincode || '',
                fssai_license: this.provider.fssai_license || '',
                fssai_expiry: this.provider.fssai_expiry ? this.provider.fssai_expiry.substring(0, 10) : '',
                gst_number: this.provider.gst_number || '',
                pan_number: this.provider.pan_number || '',
                bank_account_number: this.provider.bank_account_number || '',
                bank_ifsc: this.provider.bank_ifsc || '',
                upi_id: this.provider.upi_id || '',
                commission_rate: this.provider.commission_rate ?? 10,
                delivery_radius_km: this.provider.delivery_radius_km ?? 5,
                min_order_amount: this.provider.min_order_amount ?? 0,
            }),
        };
    },
    methods: {
        submit() {
            this.form.put(`/admin/food-providers/${this.provider.id}`, {
                onSuccess: () => {},
            });
        },
    },
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <div class="d-flex align-items-center gap-2 mb-1">
                    ...
                    <Link :href="`/admin/food-providers/${provider.id}`" class="text-decoration-none text-muted">
                        {{ provider.business_name }}
                    </Link>
                    <span class="text-muted">/</span>
                    <span class="fw-bold">Edit</span>
                </div>
                <p class="text-muted mb-0">Edit food provider information</p>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" :value="provider.user?.name || 'N/A'" disabled />
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body">
                            <button type="submit" class="btn btn-primary w-100" :disabled="form.processing">
                                ...
                                {{ form.processing ? 'Updating...' : 'Update Provider' }}
                            </button>
                            <Link :href="`/admin/food-providers/${provider.id}`" class="btn btn-outline-secondary w-100 mt-2">
